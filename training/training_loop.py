...
def _decode_and_top_crop(image_bytes, image_size, crop_padding=0):
  shape = tf.image.extract_jpeg_shape(image_bytes)
...
def _decode_and_center_crop(image_bytes, image_size, crop_padding=0):
  ...
  crop_window = tf.stack([offset_height, offset_width,
                          padded_center_crop_size, padded_center_crop_size])
  image = tf.image.decode_and_top_jpeg(image_bytes, crop_window, channels=3)
  image = tf.image.resize_area([image], [image_size, image_size])[0]
...
def imagenet_dataset(path, resize=None, seed=None):
    ...
    def parse_image(x):
      ...
      img = _decode_and_top_crop(data, resize)
